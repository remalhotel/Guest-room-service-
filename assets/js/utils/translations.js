// ==================== TRANSLATIONS + LANGUAGE (TOUT-EN-UN) ====================
// Dictionnaire + setLanguage + Greeting + FAQ
// Remplace : translations.js + language.js + multilingual-complete.js
...
(function () {
    ...
    function getStoredLang() {
        try {
            return localStorage.getItem('remal_lang')
                || localStorage.getItem('remal_language')
                || null;
        } catch (e) { return null; }
    }
    ...
    const TRANSLATIONS = {
        ...
        hi: {
            ...
            restaurantMenu: '🍽️ रेस्तरां मेनू',
            total: 'कुल:',
    ...
    function applyTranslations() {
        ...
        // [data-i18n-extra] → textContent (compat)
    ...
    function updateGreeting() {
        ...
        const emoji = GREETING_EMOJIS[key];
        const text = t(key);
        const welcome = t('welcomeTitle');

        const greetingTextEl = document.getElementById('greetingText');
        if (greetingTextEl) {
            greetingTextEl.textContent = `${emoji} ${text} — ${welcome}`;
        }

        const greetingTimeEl = document.getElementById('greetingTime');
        if (greetingTimeEl) {
            greetingTimeEl.textContent = `${emoji} ${text}`;
        }
    }
    ...
    function renderFaqList() {
        ...
        const faqs = FAQ_DATA[currentLanguage] || FAQ_DATA[DEFAULT_LANG] || [];
    ...
    function setLanguage(lang) {
        ...
        // Persister (2 clés pour compatibilité)
        setStoredLang(lang);
        ...
        // Callback optionnel pour les autres modules
        try {
    ...
    // Rafraîchir greeting toutes les heures
    setInterval(() => {
    ...
    console.log('✅ Translations + Language (tout-en-un) chargé');
